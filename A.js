...
$(document).ready(function () {
  // Hide all the circle / x's
  $("td i").hide();

  // When someone clicks on the button, with player vs player button
  $("button.modePVP").click(function () {
    // append to the header, it's player one's turn now
    $("h3").html("It's Player 1's turn now! Be quick!");
    unbindButton("button.modePVP");

    // when someone clicks on a particular box
    $("td").click(function () {
      // and if the current player is 1
      if (curPlayer == 1) {
        // add the circle to that box
        ...
        // otherwise, add a cross to that box
  ...
  $("button.modePVSAI").click(function () {
    ...
    $("td").click(function () {
      ...
      $(this).addClass("P1");
      $(this).unbind("click");
      // grab a particular index, and make it into a 1
      ticArr[Number($(this).attr("id"))] = 1;
      // Change player's turn
      curPlayer = 1;
      // run the checkFinalWin function
      checkFinalWin();
      if (won == 1) {
        return;
      }
      ...
      // make the action of appending the circle or cross slower
      setTimeout(function () {
  ...
  $("button.modePVMAI").click(function () {
    ...
    $("td").click(function () {
      ...
      // make the function fade in at a particular time
      setTimeout(function () {
        $(`td:eq(${i})`).children(".fas.fa-times").fadeIn();
        $(`td:eq(${i})`).addClass("P2");
        $(`td:eq(${i})`).unbind("click");
...
/**********************************************
 * The button deselects after you choose a mode
 * ==================================
 ***********************************************/
...
/**********************************************
 * Check the win between three boxes
 * ==================================
 ***********************************************/
function checkWin(b1, b2, b3) {
  console.log("Check win function here");
  let b = $(`td:eq(${b1})`).attr("class");
  if (
    b == $(`td:eq(${b2})`).attr("class") &&
    b == $(`td:eq(${b3})`).attr("class") &&
    b != undefined
  ) {
    $("h3").css("color", "black");
    $("h3").css("font-size", "50px");
    if (
      $("h2").html() == "Currently in Player vs Player mode"
    ) {
      $("h3").html(
        `Player ${curPlayer} win (^o^/) Congrat!`
      );
    } else if (curPlayer == 2) {
      $("h3").html(
        "Oh, my AI beat you! Maybe next time he can conquer the world!"
      );
    } else {
...
function GodAI() {
  ...
  if (
    (ticArr[3] == 1 && ticArr[4] == 2 && ticArr[5] == 1) ||
    (ticArr[1] == 1 && ticArr[4] == 2 && ticArr[7] == 1)
  ) {
    if (ticArr[0] == 0) {
      return 0;
    } else if (ticArr[2] == 0) {
